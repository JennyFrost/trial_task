    ...
    for token in doc:
        if token.pos_ in ('VERB', 'AUX'):
            if token.dep_ == 'amod' and token.head.dep_ != 'ROOT':
                continue
            if token.i >= 2:
                 if (re.search(r'ed\b', token.text) and (((doc[token.i-1].text == ',' and doc[token.i-2].pos_ == 'ADJ')
                         or doc[token.i-1].pos_ == 'ADJ') or (token.dep_ in ('conj','appos') and not re.search(r'ed\b', token.head.text)))):
                    continue
            verbs.append(token)
        elif token.pos_ == 'ADJ' and bool(re.search(r'ed\b|ing\b', token.text)) and token.head.dep_ in ('ROOT', 'nsubj') \
                and not ((doc[token.i - 1].text == ',' and doc[token.i - 2].pos_ == 'ADJ') or doc[token.i - 1].pos_ == 'ADJ'):
            verbs.append(token)
    return verbs
